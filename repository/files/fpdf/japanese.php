<?php
require_once WEBAPP_DIR.'/modules/repository/files/fpdf/fpdf.php';

$GLOBALS['SJIS_widths'] = array(' '=>278,'!'=>299,'"'=>353,'#'=>614,'$'=>614,'%'=>721,'&'=>735,'\''=>216,
	'('=>323,')'=>323,'*'=>449,'+'=>529,','=>219,'-'=>606,'.'=>219,'/'=>453,'0'=>614,'1'=>614,
...
function AddSJISFont($name='MSMincho',$family='SJIS')
{
	// Add font 2012/05/15 A.Suzuki
	// 引数に$nameを追加、以下が指定可能
	// 1) MSPMincho
    // 2) MSMincho
    // 3) MSPGothic
    ...
    if($name == 'MSPMincho' || $name == 'MSPGothic')
    {
...
function AddUniJISFont($name='MSMincho',$family='UniJIS')
{
    // Add font 2012/05/15 A.Suzuki
    // 引数に$nameを追加、以下が指定可能
    // 1) MSPMincho
...
function MultiCell($w, $h, $txt, $border=0, $align='L', $fill=false, $maxNl=0)
{
	if($this->CurrentFont['type']=='Type0')
	    if($this->CurrentFont['CMap']=='UniJIS-UTF16-H')
             $this->UniJISMultiCell($w,$h,$txt,$border,$align,$fill,$maxNl);
        else
		     $this->SJISMultiCell($w,$h,$txt,$border,$align,$fill,$maxNl);
	else
		parent::MultiCell($w,$h,$txt,$border,$align,$fill);
}

function SJISMultiCell($w, $h, $txt, $border=0, $align='L', $fill=false, $maxNl=0)
{
    $breakFlag = false;
...
function UniJISMultiCell($w, $h, $txt, $border=0, $align='L', $fill=false, $maxNl=0)
{
    $breakFlag = false;
...
function Write($h, $txt, $link='', $maxNl=0)
{
	if($this->CurrentFont['type']=='Type0')
	    if($this->CurrentFont['CMap']=='UniJIS-UTF16-H')
            $this->UniJISWrite($h,$txt,$link, $maxNl);
        else
            $this->SJISWrite($h,$txt,$link, $maxNl);
	else
		parent::Write($h,$txt,$link);
}

function SJISWrite($h, $txt, $link, $maxNl=0)
{
    $breakFlag = false;
...
function UniJISWrite($h, $txt, $link, $maxNl=0)
{
    $breakFlag = false;
